...
public class UrunStokDurumu {
    ...
    */
        ...
        for (Map.Entry<String,Integer> each:urunStokDurumuMap.entrySet()
            ...
            String urunisim = each.getKey();
            int adet = each.getValue();
            if (urunisim.equalsIgnoreCase(urun)){
                if (sayi > adet) {
                    System.out.println("Stoktaki urun adetimizi astiniz ... Urun adeti :" + adet);
                }else {
                    adet = adet - sayi;
                    each.setValue(adet);
                    System.out.println(urunStokDurumuMap);
